...
namespace Enki
{
	...
	class SimpleDisplayList : public EnkiWidget::ViewerUserData
	{
	public:
		GLuint list;
	...
	EnkiWidget::CustomRobotModel::CustomRobotModel()
	{
		deletedWithObject = false;
	}
	
	//! Create a camera at 0
	EnkiWidget::CameraPose::CameraPose():
		altitude(0),
		yaw(0),
		pitch(0)
	{
	...
	EnkiWidget::CameraPose::CameraPose(const World *world):
	...
	EnkiWidget::CameraPose::CameraPose(const QPointF& pos, double altitude, double yaw, double pitch):
		pos(pos),
		altitude(altitude),
		yaw(yaw),
		pitch(pitch)
	...
	EnkiWidget::UpdatableCameraPose::UpdatableCameraPose():
		userYaw(0),
		radius(20)
	{
		update();
	...
	EnkiWidget::UpdatableCameraPose::UpdatableCameraPose(const World *world):
		CameraPose(world),
		userYaw(M_PI/2),
		radius(20)
	{
	...
	EnkiWidget::UpdatableCameraPose::UpdatableCameraPose(const QPointF& pos, double altitude, double yaw, double pitch):
	...
	EnkiWidget::UpdatableCameraPose& EnkiWidget::UpdatableCameraPose::operator=(const CameraPose& pose)
	{
	...
	void EnkiWidget::UpdatableCameraPose::update()
	{
		yaw = userYaw;
	...
	void EnkiWidget::UpdatableCameraPose::updateTracking(double targetAngle, const QVector3D& targetPosition, double zNear)
	{
	...
	EnkiWidget::InfoMessage::InfoMessage(const QString& message, double persistance, const QColor& color, const QUrl& link):
		message(message),
	...
	EnkiWidget::EnkiWidget(World *world, QWidget *parent) :
		QGLWidget(parent),
		timerPeriodMs(30),
		camera(world),
		doDumpFrames(false),
	...
	World* EnkiWidget::getWorld() const
	{
		return world;
	}
	
	EnkiWidget::CameraPose EnkiWidget::getCamera() const
	{
		return camera;
	}

	QVector3D EnkiWidget::getPointedPoint() const
	{
		return pointedPoint;
	}

	PhysicalObject* EnkiWidget::getPointedObject() const
	{
		return pointedObject;
	}

	PhysicalObject* EnkiWidget::getSelectedObject() const
	{
		return selectedObject;
	}

	bool EnkiWidget::isTrackingActivated() const
	{
		return trackingView;
	}

	bool EnkiWidget::isMovableByPicking(PhysicalObject* object) const
	{
		if (!object)
			return 0;
	...
	*/
	void EnkiWidget::setMovableByPicking(PhysicalObject* object, bool movable)
	{
		objectExtendedAttributesList[object].movableByPicking = movable;
	}

	void EnkiWidget::setCamera(const QPointF& pos, double altitude, double yaw, double pitch)
	{
		camera.pos = pos;
	...
	void EnkiWidget::setCamera(double x, double y, double altitude, double yaw, double pitch)
	{
		setCamera(QPointF(x,y), altitude, yaw, pitch);
	}
	
	void EnkiWidget::restartDumpFrames()
	{
		dumpFramesCounter = 0;
	}
	
	void EnkiWidget::setDumpFrames(bool doDump)
	{
		doDumpFrames = doDump;
	}
	
	void EnkiWidget::setTracking(bool doTrack)
	{
	...
	void EnkiWidget::toggleTracking()
	{
		setTracking(!trackingView);
	}

	void EnkiWidget::addInfoMessage(const QString& message, double persistance, const QColor& color, const QUrl& link)
	{
	...
	void EnkiWidget::renderSegment(const Segment& segment, double height)
	{
	...
	void EnkiWidget::renderSegmentShadow(const Segment& segment, double height)
	{
		Vector v = segment.b - segment.a;
		Vector vu = v.unitary();
		Vector n = Vector(v.y, -v.x).unitary();
	...
	void EnkiWidget::renderInterSegmentShadow(const Vector& a, const Vector& b, const Vector& c, double height)
	{
		Vector prev_v = b - a;
	...
	void EnkiWidget::renderWorldSegment(const Segment& segment)
	{
		Vector v = segment.b - segment.a;
		Vector vu = v.unitary();
		Vector n = Vector(v.y, -v.x).unitary();
	...
	void EnkiWidget::renderShape(const Polygon& shape, const double height, const Color& color)
	{
	...
	void EnkiWidget::renderSimpleObject(PhysicalObject *object)
	{
	...
	void EnkiWidget::renderObjectsTypesHook()
	{
	
	}
	
	//! Called inside the creation of the object display list in local object coordinate
	void EnkiWidget::renderObjectHook(PhysicalObject *object)
	{
	...
	void EnkiWidget::displayObjectHook(PhysicalObject *object)
	{
	
	}
	
	//! Called when the drawing of the scene is completed.
	void EnkiWidget::sceneCompletedHook()
	{
		/*// overlay debug info
		if (mouseGrabbed)
	...
	void EnkiWidget::renderScene(double left, double right, double bottom, double top, double zNear, double zFar)
	{
	...
	void EnkiWidget::picking(double left, double right, double bottom, double top, double zNear, double zFar)
	{
		pointedObject = 0;
	...
	void EnkiWidget::glVertex2Screen(int x, int y)
	{
		glVertex2f(-1. + (x * 2.) / width(), 1. - (y * 2.) / height());
	}
	
	void EnkiWidget::displayWidgets()
	{
		glEnable(GL_BLEND);
		glEnable(GL_TEXTURE_2D);
		glMatrixMode(GL_PROJECTION);
	...
	void EnkiWidget::displayMessages()
	{
		// bound message list
	...
	void EnkiWidget::computeInfoMessageAreaSize()
	{
		messageListWidth = 0;
		for (MessageList::iterator it = messageList.begin(); it != messageList.end(); ++it)
	...
	void EnkiWidget::resizeGL(int width, int height)
	{
	...
	void EnkiWidget::mouseReleaseEvent(QMouseEvent * event)
	{
	...
	void EnkiWidget::mouseMoveEvent(QMouseEvent *event)
	{
		if (!trackingView && selectedObject)
		{
			// object movements
	...
	void EnkiWidget::mouseDoubleClickEvent(QMouseEvent *event)
	{
		setTracking(true);
	}
	
	void EnkiWidget::wheelEvent(QWheelEvent * event)
	{
		// zoom
		if (trackingView)
		{
	...
	void EnkiWidget::timerEvent(QTimerEvent * event)
	{
